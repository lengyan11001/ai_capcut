// gc-running.js - 群控：执行列表
    ...
    function renderRunningList() {
      ...
      var html = pageList.map(function(p) {
        var isCurrent = p.is_current_round !== false;
        var isHistory = !isCurrent;
        var statusColor = p.plan_status === 'active' ? '#4ade80' : p.plan_status === 'approved' ? '#facc15' : p.plan_status === 'paused' ? '#d97706' : p.plan_status === 'completed' ? '#6b7280' : '#9ca3af';
        var items = p.items || [];
        ...
        var schedCnt = items.filter(function(i) { return i.status === 'scheduled'; }).length;
        ...
        if (schedCnt && !isHistory) summary += ' <span style="color:#888;">' + schedCnt + '待执行</span>';
        summary += '</span>';

        var expandKey = p.plan_id + '_' + (p.round || 1);
        var expanded = !!runningExpandedKeys[expandKey];
        var arrow = expanded ? '&#9660;' : '&#9654;';
        var roundLabel = p.round_total > 1 ? '<span class="meta" style="margin-left:0.3rem;">[第' + p.round + '/' + p.round_total + '轮]</span>' : '';
        var headerOpacity = isHistory ? 'opacity:0.6;' : '';

        var header = '<div class="running-plan-header" data-expand-key="' + expandKey + '" style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;user-select:none;' + headerOpacity + '">'
        ...
        var bgOpacity = isHistory ? 'background:rgba(255,255,255,0.015);' : 'background:rgba(255,255,255,0.03);';
        return '<div style="' + bgOpacity + 'border-radius:var(--radius-sm);padding:0.6rem 0.75rem;margin-bottom:0.5rem;">' + header + detail + '</div>';
      }).join('');
      ...
      if (pageCount > 1) {
        html += '<div style="display:flex;gap:0.4rem;align-items:center;justify-content:flex-end;margin-top:0.5rem;">'
          ...
          + '<span class="meta">第 ' + runningPage + ' / ' + pageCount + ' 页（共' + total + '条记录）</span>'
      ...
      el.querySelectorAll('.running-plan-header').forEach(function(hdr) {
        hdr.addEventListener('click', function() {
          var key = hdr.getAttribute('data-expand-key');
          if (runningExpandedKeys[key]) { delete runningExpandedKeys[key]; }
          else { runningExpandedKeys[key] = true; }
          renderRunningList();
        });
      });
